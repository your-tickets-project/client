...
import handler from 'pages/api/v1/ticket';
// http status codes
import {
...
const order1 = {
  ...
  date_start: `2023-05-22T00:00:00`,
  time_start: '09:00:00',
  title: 'test title',
};
...
describe('GET route -- /api/v1/ticket -- success request', () => {
  ...
  it('should return the filtered data', async () => {
    ...
    const res = await request({
      handler,
      headers: { authorization: `Bearer ${token}` },
      url: `/api/v1/ticket?q=${order3.first_name}`,
    });
...
describe('GET route -- /api/v1/ticket -- bad request', () => {
